...
interface RegionHolidaysClientProps {
  countryCode: string;
  countryName: string;
  regionCode: string;
  regionName: string;
...
export default function RegionHolidaysClientPage({ 
  ...
  useEffect(() => {
    ...
    const fetchHolidays = async () => {
      ...
      try {
        const response = await fetch(`/api/holidays?country=${countryCode}&region=${regionCode}&year=${selectedYear}&language=${selectedLanguage}`);
    ...
    fetchHolidays();
  }, [countryCode, regionCode, selectedYear, selectedLanguage, initialYear, initialLanguage]);
    ...
    <div>
      ...
      <div>
        {loading ? (
          ...
          <div className="text-center py-8">No holidays found for {regionName}, {countryName} in {selectedYear}.</div>
        ) : (
